    ...
    switch (action.type) {
      case "SELECT":
        const selectedCards = state.cards.map((card: Card) => {
          if (card.value === action.payload.value && card.suit === action.payload.suit) {
            return {...card, selected: !card.selected}
          }
        });
        return {cards: selectedCards} as HandState
      case "REMOVE":
        const filteredCards = state.cards.filter((card: Card) => card.value !== action.payload.value && card.suit !== action.payload.suit)
